...
void main() {
  test(
    'Stylist card keeps only product-like candidates and excludes raw original',
    () {
      ...
      expect(
        candidates.map((e) => e.url),
        isNot(contains(item['originalImageUrl'])),
      );
    },
  );

  test(
    'Stylist card has no raw-person fallback when derivatives are absent',
    () {
  ...
  test('Stylist card uses Storage SDK bytes before URL fallback', () {
    final source = File('lib/widgets/stylist_suggested_item_card.dart')
        .readAsStringSync();
