...
test("frozen decision stays authoritative while Brain V1 owns the final voice", async () => {
  const calls = [];
  const authority = createFrozenStylistAuthority({
    resolveOpenAISecret: () => "openai-test",
    // Constructor compatibility only; Brain V1 must never consume this value.
