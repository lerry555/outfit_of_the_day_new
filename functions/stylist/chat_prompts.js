...
const BRAIN_CORE_TONE =
  ...
  `- Pri intent=none musí byť operations=[]. Pri create_outfit zapíš každú explicitnú požiadavku ako action=add v príslušnom slote; napr. „daj mi kraťasy a červené tenisky“ = add bottom constraints.family=shorts A add shoes constraints.color=red. Tieto create požiadavky sú tvrdé a nie sú legacy hinty. Pri generickej požiadavke bez konkrétneho kusu môže create_outfit zostať operations=[].\n` +
  `- Pri edit_current_outfit daj operáciu pre každý explicitne zmenený slot; povolené sloty sú top,bottom,shoes,layers,outerwear,full_body,accessories a action je preserve|replace|add|remove. Konfliktné replace/remove v jednom slote nevytváraj. Viac add operácií v tom istom addable slote je povolených, keď user explicitne žiada viac kusov (napr. dve rôzne accessories).\n` +
  ...
  `- Pri edit_current_outfit layers znamená iba strednú/pridanú vrstvu, nikdy skin_base/termoprádlo; outerwear znamená bundu/kabát/shell. Edit action=add je povolené iba pre layers,outerwear,accessories. Pri create_outfit je add deklarácia tvrdej požiadavky a môže byť aj top,bottom,shoes alebo full_body.\n` +
