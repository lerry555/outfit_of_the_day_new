...
function bounded(values, limit = MAX_COLLECTION) {
  return [...new Set(Array.isArray(values) ? values : [])].slice(-limit);
}

function emptyTerrain() {
  return {surface: null, difficulty: null, condition: null};
}
...
function createEmptySessionStateV2(chatId) {
  ...
  return deepFreeze({
    ...
    currentOutfit: {
      ...
      selectionReasonHistory: [],
    },
    conversationMemory: {
      pendingQuestion: null,
      pendingAction: null,
      answeredClarificationFields: {},
      communicatedWarnings: [],
      rejectedWardrobeItemIds: [],
      rejectedShoppingOptionIds: [],
      userCorrections: [],
      acceptedCompromises: [],
    },
    shopping: {
      ...
      size: null,
      budget: null,
      brandPreferences: {preferred: [], blocked: []},
      rejectedCandidateIds: [],
      openedReason: null,
    },
    wardrobePreferences: {
      wardrobeRevision: null,
...
function validateLocationObservation(value, fieldName) {
  ...
  if (typeof value !== "object" || !value.providerId || !value.label) {
    throw new TypeError(`${fieldName} requires providerId and label`);
  }
  if (fieldName === "currentLocationObservation" && !value.observedAt) {
    throw new TypeError("current GPS location requires observedAt freshness metadata");
  }
}

function validateStylistSessionStateV2(state) {
  if (!state || state.schemaVersion !== 2 || typeof state.chatId !== "string") {
    throw new TypeError("invalid StylistSessionStateV2 identity");
  }
  ...
  if (!terrain || !TERRAIN_SURFACES.has(terrain.surface) ||
      !TERRAIN_DIFFICULTIES.has(terrain.difficulty) ||
      !TERRAIN_CONDITIONS.has(terrain.condition)) {
  ...
  if (!Array.isArray(outfit.itemIds) || new Set(outfit.itemIds).size !== outfit.itemIds.length) {
    throw new TypeError("current outfit item IDs must be unique");
  }
  if (outfit.itemIds.length > MAX_COLLECTION ||
      !sameStringMembers(Object.keys(outfit.selectionReasonsByItemId), outfit.itemIds)) {
    throw new TypeError("selection reason keys must exactly match the bounded current outfit");
  }
  for (const itemId of outfit.itemIds) {
    const reason = outfit.selectionReasonsByItemId[itemId];
    if (reason !== null && (typeof reason !== "string" || !reason.trim())) {
      throw new TypeError(`invalid persisted selection reason for ${itemId}`);
    }
  }
  const boundedCollections = [
    outfit.compromises,
    outfit.missingWardrobeNeeds,
    outfit.selectionReasonHistory,
    state.conversationMemory.communicatedWarnings,
    state.conversationMemory.rejectedWardrobeItemIds,
    state.conversationMemory.rejectedShoppingOptionIds,
    state.conversationMemory.userCorrections,
    state.conversationMemory.acceptedCompromises,
    state.shopping.hardConstraints,
    state.shopping.softPreferences,
    state.shopping.rejectedCandidateIds,
    state.replay.turns,
  ];
  if (boundedCollections.some((values) => !Array.isArray(values) || values.length > MAX_COLLECTION)) {
    throw new TypeError("session collections must be bounded");
  }
  ...
  if (Object.keys(state.conversationMemory.answeredClarificationFields).length > MAX_COLLECTION ||
      state.shopping.brandPreferences.preferred.length > MAX_COLLECTION ||
      state.shopping.brandPreferences.blocked.length > MAX_COLLECTION) {
  ...
  if (pendingAction != null && (pendingAction.type !== "action" || !pendingAction.kind || !pendingAction.actionId)) {
    throw new TypeError("pendingAction requires one typed actionId referent");
  }
  const pendingQuestion = state.conversationMemory.pendingQuestion;
  if (pendingQuestion != null && (pendingQuestion.type !== "question" || !pendingQuestion.field ||
      !pendingQuestion.question || !pendingQuestion.actionId)) {
    throw new TypeError("pendingQuestion requires one typed question referent");
  }
...
function bootstrapExistingChatV2(input) {
  ...
  state.currentOutfit.selectionReasonsByItemId = Object.fromEntries(ids.map((itemId) => {
    const reason = persistedReasons[itemId];
    return [itemId, typeof reason === "string" && reason.trim() ? reason.trim() : null];
  }));
  ...
  // Deliberately not migrated from prose or inferred from GPS/activity.
