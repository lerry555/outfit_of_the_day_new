...
async function clientWriteDocument(idToken, path, value) {
  ...
  if (!response.ok) throw new Error(`client_firestore_write_failed:${path}:${response.status}:${text.slice(0, 600)}`);
}
...
async function main() {
  ...
  try {
    ...
    await clientWriteDocument(idToken, `users/${uid}`, {_qaStylistSmoke: true, createdAt: new Date()});
    ...
    assert.ok(Array.isArray(second.resultingOutfitItemIds) && second.resultingOutfitItemIds.length >= 2,
      `turn2_missing_outfit:${JSON.stringify(visible(second))}`);
    assert.ok(Array.isArray(second.displayItemIds) && second.displayItemIds.length >= 2,
      `turn2_missing_display:${JSON.stringify(visible(second))}`);

    console.log("LIVE_SWITZERLAND_ALPS_SMOKE_OK");
  } finally {
    if (idToken) {
      for (const [id] of wardrobe) await clientDeleteDocument(idToken, `users/${uid}/wardrobe/${id}`);
