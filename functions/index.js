...
} = require("./stylist/simple_stylist_fast_conversation_v1");
...
function fastStylistConversationForUserV1(uid, requestId) {
  return createFastConversationRouterV1({
    executeModel: createOpenAiSimpleAgentExecutorV1({
      fetchImpl: fetch,
      resolveOpenAISecret,
      logger,
      cacheScope: `${uid}:fast-conversation`,
      feature: "stylist_fast_conversation",
      recordUsage: (event) => recordAiUsageV1({...event,
        userKey: hashValue(uid), requestKey: hashValue([uid, requestId, "fast"])}),
    }),
    logger,
  });
}

function currentOutfitIdsForFastPathV1(value) {
  if (!Array.isArray(value)) return [];
  const out = [];
  const seen = new Set();
  for (const raw of value) {
    const id = String(raw || "").trim().slice(0, 180);
...
async function loadCurrentOutfitDocsForFastPathV1(uid, rawIds) {
  const ids = currentOutfitIdsForFastPathV1(rawIds);
  if (!ids.length) return [];
  const root = db.collection("users").doc(uid).collection("wardrobe");
  const snapshots = await db.getAll(...ids.map((id) => root.doc(id)));
  if (snapshots.some((snapshot) => !snapshot.exists)) return null;
  return snapshots.map((snapshot) => ({id: snapshot.id, ...(snapshot.data() || {})}));
}

function materializeFastConversationResultV1({decision, data, wardrobeItems,
  currentSelectionReasons}) {
  const request = normalizeRequestV1({
    ...
    userStylePreferences: sanitizeUserStylePreferences(data?.userStylePreferences),
  });
  const validation = validateAgentResultV1({
    stylistComment: decision.stylistComment,
    resultingOutfitItemIds: request.currentOutfitItemIds,
    displayItemIds: [],
    outfitChanged: false,
    outfitRequested: false,
    quickReplyMode: decision.quickReplyMode,
    weatherContextKey: decision.weatherContextKey,
  ...
  if (!validation.valid) {
    const error = new Error("fast_conversation_materialization_failed");
    error.validationErrors = validation.errors;
    throw error;
  }
  return Object.freeze({...materializeResultV1(validation, request),
    modelPath: "fast_conversation"});
}
    ...
    .https.onCall(async (data, context) => {
      ...
      try {
        task = await runAiTaskOnceV1({
          ...
          execute: async () => {
            try {
              const shoppingEnabled = data?.shoppingEnabled === true;
              const shoppingContext = data?.shoppingContext &&
                typeof data.shoppingContext === "object" &&
                !Array.isArray(data.shoppingContext) ? data.shoppingContext : {};
              const shoppingActive = Boolean(
                shoppingContext.sessionId || shoppingContext.activeClarification ||
                shoppingContext.pendingSourceText || shoppingContext.pendingNeedText,
              );
              if (shoppingEnabled && shoppingActive) {
                ...
                if (shoppingTurn.handled) return shoppingTurn.response;
              }

              const fastDecision = await fastStylistConversationForUserV1(
                uid, requestId,
              ).resolve({
                message: data?.message,
                history: data?.history,
              ...
              if (fastDecision.route === "fast_reply" &&
                ...
                const shoppingTurn = await handleStylistShoppingTurn({
                  ...
                  wardrobeSignal: startApproved ? null : {
                    ...
                    canonicalType: fastDecision.shoppingNeedText,
                    needLabel: fastDecision.shoppingNeedLabel,
                  },
                });
                if (shoppingTurn.handled) return shoppingTurn.response;
              }

              const {loadSelectionReasonsV1} = require(
                "./stylist/simple_stylist_choice_memory_v1",
              );
              const currentSelectionReasons =
                Array.isArray(data?.currentSelectionReasons) &&
              ...
              if (fastDecision.route === "fast_reply") {
                const currentItems = await loadCurrentOutfitDocsForFastPathV1(
                  uid, data?.currentOutfitItemIds,
                );
                if (currentItems !== null) {
                  return materializeFastConversationResultV1({
                ...
                logger.warn("SIMPLE_AGENT_FAST_ROUTE_FALLBACK", {
                  code: "current_outfit_document_missing",
                });
              }
              ...
              return Object.freeze({...result, modelPath: "full_stylist"});
            } catch (error) {
