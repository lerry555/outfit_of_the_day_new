...
/// Hlavná obrazovka so žiadosťami o zladenie outfitov.
/// Rozdelenie na "Otvorené" a "Uzavreté".
class MatchRequestsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (user == null) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Zladenie outfitov'),
        ),
        body: const Center(
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          children: [
            _MatchRequestsList(status: 'open'),
            _MatchRequestsList(status: 'closed'),
          ],
        ),
      ),
    );
...
/// Detail žiadosti – zatiaľ iba prehľad info + možnosť uzavrieť žiadosť.
...
class _MatchRequestDetailScreenState extends State<MatchRequestDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final eventType = data['eventType'] as String? ?? 'Udalosť';
    final contextText =
        data['context'] as String? ?? ''; // napr. popis udalosti / miesto
    final userMessage =
        data['userMessage'] as String? ?? ''; // čo si napísal pri žiadosti
      ...
      body: ListView(
        ...
        children: [
          ...
          ListTile(
            ...
            subtitle: Text(
              status == 'open' ? 'Otvorená – môžete sa ešte dohadovať.' : 'Uzavretá.',
            ),
          ),
          ...
          if (status == 'open')
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(
                  _isClosing ? 'Uzatváram...' : 'Označiť žiadosť ako uzavretú',
                ),
              ),
            ),
        ],
            ...
            children: [
              ...
              InkWell(
                ...
                child: InputDecorator(
                  decoration: const InputDecoration(
                    labelText: 'Dátum / čas udalosti (nepovinné)',
                    border: OutlineInputBorder(),
                  ),
                  child: Text(
                    _eventDate != null
  ...
  Future<void> _saveRequest(
    ...
    try {
      ...
      if (mounted) {
        ...
        // Voliteľne môžeme hneď otvoriť detail:
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (_) => MatchRequestDetailScreen(
              requestId: docRef.id,
