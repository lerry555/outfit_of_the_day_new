...
class _TripPackingScreenState extends State<TripPackingScreen> {
  ...
  TripTransport _transport = TripTransport.car;
  ...
  /// Spiatočná letová časť: používateľ môže prepísať; pri zmene tamojších letísk sa znova doplní swap.
  bool _planeReturnUserOverride = false;
  ...
  void _onOutboundPlaneFieldChanged() {
    _planeReturnUserOverride = false;
    _applyPlaneReturnMirror();
    _applyPlaneOutboundArrivalEstimate();
    _applyPlaneReturnArrivalEstimate();
  }

  void _onReturnPlaneFieldChanged() {
    if (_mutePlaneReturnMirror) return;
    _planeReturnUserOverride = true;
    _applyPlaneReturnArrivalEstimate();
  }

  void _applyPlaneReturnMirror() {
    if (_planeReturnUserOverride) return;
    final o = _planeOutboundOriginCtrl.text.trim();
    final d = _planeOutboundDestCtrl.text.trim();
    ...
    _mutePlaneReturnMirror = true;
    _planeReturnOriginCtrl.text = d;
    _planeReturnDestCtrl.text = o;
    _mutePlaneReturnMirror = false;
    if (mounted) {
      setState(() {
  ...
  /// Po výbere destinácie cesty alebo prepnutí na lietadlo: „Kam prilietaš“ z prvého výsledku vyhľadávania.
  Future<void> _tryPrefillOutboundArrivalAirportFromDestination() async {
    if (_transport != TripTransport.plane) return;
    if (_planeOutboundDestCtrl.text.trim().isNotEmpty || _outboundArrAirport != null) return;
    ...
    try {
      ...
      final hits = await AirportRepository.search(dest, limit: 12);
      if (!mounted || _transport != TripTransport.plane) return;
      if (_planeOutboundDestCtrl.text.trim().isNotEmpty || _outboundArrAirport != null) return;
      if (hits.isEmpty) {
        if (kDebugMode) {
          debugPrint('[TRIP_PACKING] planeArrivalPrefill: no airports for destination');
        }
        _restoreScrollOffset(saved);
        return;
      }
      ...
      if (kDebugMode) {
        debugPrint('[TRIP_PACKING] planeArrivalPrefill: dest="$dest" -> ${best.iata} ${best.name}');
      }

      _planeOutboundDestCtrl.removeListener(_onOutboundPlaneFieldChanged);
      try {
        setState(() {
        ...
        _planeOutboundDestCtrl.addListener(_onOutboundPlaneFieldChanged);
      }
      _restoreScrollOffset(saved);
      _applyPlaneReturnMirror();
      _applyPlaneOutboundArrivalEstimate();
  ...
  void initState() {
    super.initState();
    _planeOutboundOriginCtrl.addListener(_onOutboundPlaneFieldChanged);
    _planeOutboundDestCtrl.addListener(_onOutboundPlaneFieldChanged);
    _planeReturnOriginCtrl.addListener(_onReturnPlaneFieldChanged);
    _planeReturnDestCtrl.addListener(_onReturnPlaneFieldChanged);
  ...
  void dispose() {
    _outboundFlightDebounce?.cancel();
    _returnFlightDebounce?.cancel();
    _planeOutboundOriginCtrl.removeListener(_onOutboundPlaneFieldChanged);
    _planeOutboundDestCtrl.removeListener(_onOutboundPlaneFieldChanged);
    _planeReturnOriginCtrl.removeListener(_onReturnPlaneFieldChanged);
    _planeReturnDestCtrl.removeListener(_onReturnPlaneFieldChanged);
                        ...
                        if (_result == null) ...[
                          _DestinationCard(
                            ...
                            onSuggestionTap: (value) {
                              ...
                              _prefillReturnFromDestination();
                              if (_transport == TripTransport.plane) {
                                unawaited(_tryPrefillOutboundArrivalAirportFromDestination());
                              }
                          ...
                          _TransportDetailsCard(
                            ...
                            onOutboundArrAirportChanged: (a) {
                              final s = _scrollOffsetOrZero();
                              setState(() => _outboundArrAirport = a);
                              _restoreScrollOffset(s);
                            },
                            onReturnDepAirportChanged: (a) {
                              final s = _scrollOffsetOrZero();
                              setState(() => _returnDepAirport = a);
                              _restoreScrollOffset(s);
                            },
                            onReturnArrAirportChanged: (a) {
                              final s = _scrollOffsetOrZero();
                              setState(() => _returnArrAirport = a);
                              _restoreScrollOffset(s);
                            },
    ...
    if (outboundFlightEstimate != null &&
      ...
      return [
        ...
        _AutoArrivalEstimateCard(
          ...
          arrivalLocalCity: airportOutboundArr?.city,
          subtitle:
    ...
    if (returnFlightEstimate != null &&
      ...
      return [
        ...
        _AutoArrivalEstimateCard(
          ...
          arrivalLocalCity: airportReturnArr?.city,
          subtitle:
              'Približný čas letu: ${formatBlockDurationSk(fe.duration.estimatedDurationMinutes)}',
          warnings: warns,
        ),
