...
class StylistSimpleAgentResultV1 {
  ...
  factory StylistSimpleAgentResultV1.fromCallableData(Map data) {
    ...
    final quickReplyPromptRaw = (data['quickReplyPrompt'] ?? '')
        .toString()
        .trim();
    return StylistSimpleAgentResultV1(
      ok: true,
      ...
      quickReplyPrompt: quickReplyPromptRaw.isEmpty
          ? null
          : quickReplyPromptRaw,
  ...
  Map<String, dynamic> toUiResponse() => <String, dynamic>{
    ...
    if (quickReplyPrompt != null) 'quickReplyPrompt': quickReplyPrompt,
...
class StylistSimpleAgentServiceV1 {
  ...
  Future<void> _loadStylePreferences() async {
    try {
      final prefs = await _stylePreferences
          .loadForUid(_auth.currentUser?.uid)
          .timeout(const Duration(seconds: 3));
      _cachedStylePreferencesPayload = StylePreferencesRuntime.stylistPayload(
        prefs,
      );
    } catch (_) {
  ...
  Future<Map<String, dynamic>> sendTurn({
    ...
    debugPrint(
      'SIMPLE_AGENT_REQUEST history=${history.length} current=${currentOutfitItemIds.length}',
    );
    try {
      final callable = FirebaseFunctions.instanceFor(region: 'us-east1')
          .httpsCallable('stylistChatV2');
      ...
      final targetSessionId = realChatId.isNotEmpty
          ? realChatId
          : _provisionalV2SessionId;
      final previousSessionId =
          realChatId.isNotEmpty &&
      ...
      if (stylePayload != null)
        payload['userStylePreferences'] = Map<String, dynamic>.from(
          stylePayload,
        );
      ...
      if (_shoppingActions.contains((rawData['action'] ?? '').toString())) {
        debugPrint(
          'SIMPLE_AGENT_SHOPPING_HANDOFF action=${rawData['action']} latencyMs=${stopwatch.elapsedMilliseconds}',
        );
        return rawData;
      }
      debugPrint(
        'SIMPLE_AGENT_RESULT received=true path=${rawData['modelPath'] ?? 'unknown'} latencyMs=${stopwatch.elapsedMilliseconds}',
      );
      ...
      if (result.failClosed) {
        ...
        debugPrint(
          'SIMPLE_AGENT_VALIDATED result=${result.resultingOutfitItemIds} display=${result.displayItemIds}',
        );
      }
      return result.toUiResponse();
    } catch (error, stackTrace) {
      debugPrint(
        'SIMPLE_AGENT_FAIL_CLOSED client=$error latencyMs=${stopwatch.elapsedMilliseconds}',
      );
