...
import 'destination_airport_matcher.dart';
...
abstract final class AirportRepository {
  ...
  static List<AirportRecord> get allAirports => List.unmodifiable(_cache ?? const []);
  ...
  static List<String> _tokenize(String norm) {
    final cleaned = norm.replaceAll(RegExp(r'[^\w\s]'), ' ');
    return cleaned
        .split(RegExp(r'\s+'))
        .map((t) => t.trim())
        .where((t) => t.length >= 2)
  ...
  static double _scoreAirport(
    ...
    for (final t in tokens) {
      ...
      if (t.length == 3 && iata == t) score += 1e9;
      if (t.length >= 4 && iata.contains(t)) score += 50000;
      if (t.length >= 3 && city == t) score += 400000;
      if (t.length >= 3 && city.contains(t) && t.length >= 4) score += 80000;
      if (t.length >= 4 && name.contains(t)) score += 35000;
      if (t.length >= 4 && country.contains(t)) score += 15000;
    }
    ...
    if (score > 0) {
      ...
      if (name.contains('international')) score += 400;
    }
    return score;
  }
  ...
  /// Najlepšie letisko pre zadanú destináciu cesty (trip prefill).
  static Future<AirportRecord?> matchBestForTripDestination({
    ...
    String? featureCode,
      ...
      DestinationAirportMatcher.matchBest(
